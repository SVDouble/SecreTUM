...
class InstrumentManager:
            ...
            for i in range(start, start + count):
                ...
                for array in arrays:
                    ...
                    if array_type == pspydata.ArrayType.Frequency:
                        data["frequency"] = pspydata._get_values_from_NETArray(
                            array, start=i, count=1
                        )
                    elif array_type == pspydata.ArrayType.ZRe:
                        data["zre"] = pspydata._get_values_from_NETArray(
                            array, start=i, count=1
                        )
                    elif array_type == pspydata.ArrayType.ZIm:
                        data["zim"] = pspydata._get_values_from_NETArray(
                            array, start=i, count=1
                        )
    ...
    def __eis_data_finished(self, eisdata):
        ...
        if eisdata.NPoints - self.__index_last_sent_point > 2:
            self.__eis_data_update(
                eisdata,
                self.__index_last_sent_point + 1,
                eisdata.NPoints - self.__index_last_sent_point,
            )
        return
    ...
    def __curve_update(self, curve, start, count):
        ...
        if self.new_data_callback is not None:
            for i in range(start, start + count):
                data = {}
                data["index"] = i + 1
                data["x"] = pspydata._get_values_from_NETArray(
                ...
                data["y"] = pspydata._get_values_from_NETArray(
